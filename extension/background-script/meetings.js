...
import { sendMeetingToAnalysisServer } from './analysis.js'
            ...
            .then(() => {
                chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
                    ...
                    chrome.storage.sync.get(["webhookUrl", "autoPostWebhookAfterMeeting", "autoDownloadFileAfterMeeting"], function (resultSyncUntyped) {
                        ...
                        // Keep the local transcript workflow reliable if the optional report service is offline.
                        promises.push(sendMeetingToAnalysisServer(lastIndex).catch((error) => {
                            console.error("AI email report failed:", error)
                            return "AI email report failed"
                        }))
