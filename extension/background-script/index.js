...
chrome.runtime.onMessage.addListener(function (messageUnTyped, sender, sendResponse) {
    ...
    if ((message.type === "enable_platform")) {
        ...
        requestPlatformPermission(platform).then(() => {
            // Register the selected Google Meet content script after permissions are granted.
    ...
    if (message.type === "create_github_issue") {
        ...
        fetch("http://127.0.0.1:8080/github/create-issue", {
            method: "POST",
            ...
            .then(async (res) => {
                ...
                /** @type {ExtensionResponse} */
                const response = { success: true, message: data.issue_url, issueUrl: data.issue_url }
                sendResponse(response)
            })
            .catch((err) => {
    ...
    if (message.type === "create_calendar_reminder") {
        const command = message.command || ""
        fetch("http://127.0.0.1:8080/calendar/set-reminder", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body: `command=${encodeURIComponent(command)}`
        })
            .then(async (res) => {
                ...
                if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`)
                /** @type {ExtensionResponse} */
                const response = { success: true, message: data.web_link, webLink: data.web_link }
                sendResponse(response)
            })
            .catch((err) => {
...
chrome.permissions.onAdded.addListener((event) => {
    ...
    setTimeout(() => {
        /** @type {Platform[]} */
        const platforms = ["google_meet"]
        /** @type {Promise<any>[]} */
        const deRegisterPromises = [deregisterContentScript(platforms)]
...
chrome.runtime.onInstalled.addListener(() => {
    ...
    chrome.storage.sync.get(null, function (resultSyncUntyped) {
        ...
        chrome.storage.sync.set({
            ...
            autoEmailReportAfterMeeting: resultSync.autoEmailReportAfterMeeting === false ? false : true,
        }, function () {
