            ...
            .then((targetNode) => {
                if (targetNode) {
                    ...
                    chrome.storage.sync.get(["operationMode"], function (resultSyncUntyped) {
                        const resultSync = /** @type {ResultSync} */ (resultSyncUntyped)
                        if (resultSync.operationMode === "manual") {
                            showNotificationGoogleMeet({ status: 400, message: "<strong>Transcriber is not running</strong> <br /> Turn on captions using the CC icon, if needed" })
                        }
                        else {
                            showNotificationGoogleMeet(state.extensionStatusJSON)
                        }
        ...
        try {
            ...
            if (chatMessagesElement && chatMessagesElement.children.length > 0) {
                ...
                if (personName && chatMessageText) {
                    ...
                    // Detect CREATE ISSUE command and dispatch to background — fires exactly once per unique command
                    if (/^\s*CREATE\s+ISSUE\s+@me/i.test(chatMessageText) && !state.dispatchedCommands.has(chatMessageText)) {
                        state.dispatchedCommands.add(chatMessageText)
                        /** @type {ExtensionMessage} */
                        const issueMessage = {
                            type: "create_github_issue",
                            command: chatMessageText.trim()
                        }
                        chrome.runtime.sendMessage(issueMessage, (response) => {
                            if (response?.success) {
                                console.log(`GitHub issue created: ${response.issueUrl}`)
                            } else {
                                console.warn("GitHub issue creation failed:", response?.error)
                            }
                        })
                    }

                    // Detect SET REMINDER command and dispatch to background — fires exactly once per unique command
                    if (/^\s*SET\s+REMINDER\s+/i.test(chatMessageText) && !state.dispatchedCommands.has(chatMessageText)) {
                        state.dispatchedCommands.add(chatMessageText)
                        /** @type {ExtensionMessage} */
                        ...
                        chrome.runtime.sendMessage(reminderMessage, (response) => {
                            if (response?.success) {
                                console.log(`Calendar reminder created: ${response.webLink}`)
                            } else {
                                console.warn("Calendar reminder creation failed:", response?.error)
                            }
                        })
                    }
                }
